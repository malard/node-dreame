...
describe("Vacuum.refresh", () => {
  it("returns kind=online and applies values when the cloud responds", async () => {
  ...
  it("returns kind=offline and forces state.online=false on 80001", async () => {
    const stub: DreameClient["getProperties"] = async () => {
    ...
    expect(result.state.online).toBe(false);
